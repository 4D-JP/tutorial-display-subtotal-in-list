...
リストボックスとは違い，リストフォームのデータソースには命名セレクションを設定することができません。フォームの右側には，```[InvoiceDetail]```をソースとするサブフォームが配置されていますが，このフォームは，隣接するリストボックスの```On Display Detail```内で実行された```RELATE MANY```の影響を受けることになります。

具体的には，

*  ```RELATE MANY```により，セレクションの先頭レコードがカレントレコードになる
* サブフォームに表示された縦スクロールバーの位置が正確に描画されないことがある

もちろん，同一フォームに複数のリストフォームを配置したり，一方のカレントセレクションに他方の```On Display Detail```などが干渉したりしなければ，こうした問題は発生しません。
...
* データソース《式》にメソッドなどを記述する，または
* ```On  Display Detail```でメソッドなどを実行する
