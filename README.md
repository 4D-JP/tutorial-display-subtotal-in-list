...
Searchエリアに番号を入力すると，```[Invoice]```のセレクションが絞り込まれます。各行には，```[Invoice]```にリンクしている```[InvoiceDetail]```のセレクションを集計したSubtotalが表示されており，リストの一番下には，その合計値が表示されています。左側のリストで行を選択すると，その行がカレントレコードになり，右側のリストに```[InvoiceDetail]```のセレクションが表示されます。
